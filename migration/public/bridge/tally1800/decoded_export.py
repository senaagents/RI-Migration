...
def _export_best_ledger_entries(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        typed as (
          select *,
            case
              ...
              when instr(lower(coalesce(voucher_type_name, '')), 'delivery challan') > 0 then 1
              when instr(lower(coalesce(voucher_type_name, '')), 'delivery note') > 0 then 1
              else 0
        ...
        gated as (
          select *,
            case
              when instr(lower(coalesce(voucher_type_name, '')), 'stock journal') > 0
                or instr(lower(coalesce(voucher_type_name, '')), 'store movement') > 0
                then null
              when has_0006 = 1 and (has_52d3 = 1 or has_0002 = 1 or has_2713 = 1) then 'high'
              when has_52d3 = 1 then 'high'
              when has_0002 = 1 and has_0006 = 0 and has_2713 = 0
...
def _synthesize_stock_counterparts(conn: sqlite3.Connection) -> None:
    """Insert balancing counterparts for inventory-only voucher types.

    Tally voucher types like Material In / Material Out store only the party
    ledger in the voucher record; the offsetting entry to a stock-control GL
    is implicit and only materialised at trial-balance time. To make
    voucher_ledger_entries_1800 sum to zero per voucher (and across the TB),
    we synthesise the missing counterpart with ledger_master_id = NULL,
    a sentinel ledger name and confidence='synthesized'.
    """
    conn.execute(
        """
        delete from voucher_ledger_entries_1800
        where confidence = 'synthesized'
          and sources = 'inventory_balance_synthesis'
        """
    )
    conn.execute(
        """
        insert into voucher_ledger_entries_1800
          (voucher_master_id, voucher_type_master_id, voucher_type_name,
           ledger_master_id, ledger_name, amount_scaled, amount_value,
           sources, confidence, evidence_count,
           first_page_no, first_page_offset)
        with sums as (
          select voucher_master_id,
                 min(voucher_type_master_id) as voucher_type_master_id,
                 min(voucher_type_name) as voucher_type_name,
                 sum(amount_scaled) as net_scaled,
                 min(first_page_no) as first_page_no,
                 min(first_page_offset) as first_page_offset
        ...
        select
          ...
          voucher_type_master_id,
          voucher_type_name,
          null as ledger_master_id,
          '<synthesized stock counterpart>' as ledger_name,
          -net_scaled as amount_scaled,
          -net_scaled / 100000.0 as amount_value,
          'inventory_balance_synthesis' as sources,
          'synthesized' as confidence,
          0 as evidence_count,
          first_page_no,
          first_page_offset
        from sums
        where net_scaled != 0
          and (
            instr(lower(coalesce(voucher_type_name, '')), 'material in') > 0
            or instr(lower(coalesce(voucher_type_name, '')), 'material out') > 0
          )
        """
    )
...
def _export_decode_audit(conn: sqlite3.Connection) -> None:
    ...
    for (
        ...
        if candidate["unique_tuples"] > best["rows"]:
...
def export_decoded_sqlite(
    ...
    with sqlite3.connect(out_path) as conn:
        ...
        if "TranMgr.1800" in file_map:
            ...
            _synthesize_stock_counterparts(conn)
